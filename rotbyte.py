...
# (text, kind) rows for the short --help. kind selects what gets colored to
# match argparse's own --help-all palette: "heading" colors the whole line
# (section titles), "usage" colors the "usage:" label and the prog name, a
# flag string colors just that substring (the option being demonstrated),
# and None leaves the line plain — mirroring how argparse itself only
# colorizes the usage/options it generates and leaves the epilog plain.
_SHORT_HELP_LINES = [
    ("usage: rotbyte [path] [options]", "usage"),
    ("", None),
    ("Detect bit rot by tracking BLAKE2b checksums in a SQLite database.", None),
    ("", None),
    ("Common commands:", "heading"),
    ("  rotbyte                        Scan current directory", None),
    ("  rotbyte --check                Full re-verify (catches silent bit rot)", "--check"),
    ("  rotbyte --report               Show integrity status summary", "--report"),
    ("  rotbyte --accept-all           Accept current state as new baseline", "--accept-all"),
    ("  rotbyte --status               Show scheduled scans and file health", "--status"),
    ("", None),
    ("Set up automated scanning:", "heading"),
    ("  rotbyte --track-setup          Interactive wizard: schedule + budget", "--track-setup"),
    ("  rotbyte --notify-setup email   Interactive wizard: email alerts", "--notify-setup"),
    ("", None),
    ("More help:", "heading"),
    ("  rotbyte --help-all             Full option list and exit codes", "--help-all"),
    ("  rotbyte --docs                 List setup guides (notify, permissions, scheduler)", "--docs"),
    ("  man rotbyte                    Full manual (Homebrew installs)", None),
]


class _NoHelpColors:
    usage = prog = heading = long_option = reset = ""


def _help_colors():
    """Argparse's own color theme (Python 3.13+), so --help matches
    --help-all's palette exactly, including NO_COLOR/FORCE_COLOR/tty
    detection. _colorize is a private CPython module with no compatibility
    guarantee, so any failure -- missing on Python <3.13, or a shape change
    in a future version -- falls back to plain text rather than risking
    --help itself crashing."""
    try:
        from _colorize import get_theme
        return get_theme().argparse
    except Exception:
        return _NoHelpColors()


def _render_short_help() -> str:
    c = _help_colors()
    # getattr(..., "") rather than direct attribute access: _colorize is
    # private with no compatibility guarantee, so a field renamed in a
    # future CPython should degrade to plain text, not crash --help.
    heading = getattr(c, "heading", "")
    usage = getattr(c, "usage", "")
    prog = getattr(c, "prog", "")
    long_option = getattr(c, "long_option", "")
    reset = getattr(c, "reset", "")
    lines = []
    for text, kind in _SHORT_HELP_LINES:
        if kind == "heading":
            text = f"{heading}{text}{reset}"
        elif kind == "usage":
            text = text.replace("usage:", f"{usage}usage:{reset}", 1)
            text = text.replace("rotbyte", f"{prog}rotbyte{reset}", 1)
        elif kind is not None:
            text = text.replace(kind, f"{long_option}{kind}{reset}", 1)
        lines.append(text)
    return "\n".join(lines) + "\n"


class _ShortHelpAction(argparse.Action):
    """Prints the curated quick-reference help instead of argparse's full
    autogenerated listing. The full listing is still reachable via
    --help-all (registered with the standard 'help' action)."""
...
""",
    ...
    parser.add_argument("-h", "--help", action=_ShortHelpAction,
                        help="Show this help message and exit")
    parser.add_argument("--version", action="version", version=f"%(prog)s {VERSION}")
    parser.add_argument("--help-all", action="help",
                        help="Show the full option list and exit codes")
